document.addEventListener("DOMContentLoaded", () => {
    ...
    function toggleAllEditing(isEditing) {
      document
        .querySelectorAll("#attendance_table select")
        .forEach(
          (select) => (select.style.display = isEditing ? "inline" : "none")
        );
      document
        .querySelectorAll("#attendance_table span")
        .forEach((span) => (span.style.display = isEditing ? "none" : "inline"));
      document
        .querySelectorAll(".edit-btn")
        .forEach(
          (button) => (button.style.display = isEditing ? "none" : "inline")
        );
      document
        .querySelectorAll(".save-btn")
        .forEach(
          (button) => (button.style.display = isEditing ? "inline" : "none")
        );
    }
    ...
    function saveRowChanges(row) {
      ...
      if (!statusSelect || !statusSpan) {
        console.error(
          "Status select or span element not found for student number:",
          studentNo
        );
        return;
      }
      ...
      fetch("../php/update_attendance.php", {
        ...
        .then((responseText) => {
          if (responseText.trim() === "success") {
            ...
            console.error(
              "Failed to update status for student number:",
              studentNo
            );
          }
        })
        .catch((error) =>
          console.error(
            "Error updating status for student number:",
            studentNo,
            error
          )
        );
    ...
    function saveAllChanges() {
      const promises = Array.from(
        document.querySelectorAll("#attendance_table tr")
      ).map((row) => {
        const studentNo = row.getAttribute("data-student-no");
        const statusSelect = row.querySelector("select");
        const statusSpan = row.querySelector("span");
